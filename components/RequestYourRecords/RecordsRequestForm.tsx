...
const RecordsRequestForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <h2 className="text-xl font-semibold text-slate-900 mb-2">Records Request Form</h2>
      ...
      <div className="flex flex-col gap-1.5 mb-2">
        ...
        <input 
          type="date" 
          className="px-4 py-2.5 bg-gray-100 border-none rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm text-slate-600 appearance-none w-full" 
        />
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {requestTypes.map((type) => (
            ...
            >
              ...
              <span className={`text-sm font-semibold mt-1 ${selectedRequest === type.id ? 'text-blue-700' : 'text-slate-800'}`}>
                {type.title}
              </span>
      ...
      <button 
        type="submit"
        className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-6 rounded-full transition-colors text-sm w-fit mt-4"
      >
