...
export async function generateMetadata({ params }: { params: any }) {
  try {
    ...
    const res = await fetch(`https://prod.weightlossmdcherrycreek.com/api/v1/public/blogs/${slug}`, { next: { revalidate: 60 } });
    ...
    return {
      ...
      description: blog.category?.name || "Read our latest insights on medical weight management.",
    };
  } catch (error) {
...
export default async function BlogDetailsPage({ params }: { params: any }) {
  const resolvedParams = await params;
  const slug = resolvedParams.id;
  const res = await fetch(`https://prod.weightlossmdcherrycreek.com/api/v1/public/blogs/${slug}`, { next: { revalidate: 60 } }).catch(() => null);
  ...
  const publishedDate = blog.createdAt ? new Date(blog.createdAt).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric'
  }) : '';
          ...
          <div className="lg:col-span-2 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-6 mb-6 text-sm">
              <div className="text-gray-600">
                <span className="font-semibold text-gray-900">Published:</span> {publishedDate}
              </div>
